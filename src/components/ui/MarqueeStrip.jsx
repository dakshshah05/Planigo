...
export default function MarqueeStrip({ className = '' }) {
  ...
  return (
    <section className={`relative py-6 overflow-hidden border-y border-white/5 bg-white/[0.02] ${className}`}>
      {/* Row 1 — left scroll */}
      <div className="flex gap-8 w-max animate-marquee hover:[animation-play-state:paused] mb-3">
        {row}
        {row}
      </div>
      {/* Row 2 — right scroll */}
      <div className="flex gap-8 w-max animate-marquee2 hover:[animation-play-state:paused]">
        {row}
        {row}
      </div>
    </section>
